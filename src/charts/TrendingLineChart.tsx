...
const TrendingLineChart: React.FC<TrendingLineChartProps> = ({
  data,
  isDashboard = false
}) => {
  ...
  const transformedData: Array<TransformedDataPoint> =
    Array.isArray(data) && 'topic' in data[0]
      ? (data as RawDataPoint[]).reduce(
          (acc: TransformedDataPoint[], point) => {
            ...
            if (topicSeries) {
              ...
              acc.push({
                id: point.topic,
                data: [{ x: point.date, y: point.value }]
              })
            }
